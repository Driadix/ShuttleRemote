...
ScrollingListWidget::ScrollingListWidget(const char* const* items, uint8_t itemCount, uint8_t visibleItems)
    : _items(items), _itemCount(itemCount), _visibleItems(visibleItems), _cursorPos(0), _topIndex(0) {
}
...
void ScrollingListWidget::draw(U8G2& display, uint8_t x, uint8_t y) {
    ...
    for (uint8_t i = 0; i < _visibleItems; i++) {
        ...
        display.setCursor(x + 2, textY); // Small padding
        display.print(_items[realIndex]);
    }

    display.setDrawColor(1); // Restore draw color
}
